...
Эти разделы используют единую визуальную систему на основе глобальных токенов `styles.css`:
- `--mocha-dark`
- `--mocha-medium`
- `--mocha-border`
- `--cashmere`

Эквивалентные карточки используют одинаковую систему границ, радиусов, отступов и типографики. Кнопки уровня `Назад` используют одинаковое оформление: полная ширина, 48px, радиус 14px, единая граница и насыщенность шрифта.

`SERVICE`, `DOCUMENTS` и `LOYALTY` сохраняют собственные функциональные модули, но не создают отдельный визуальный язык для одинаковых элементов управления.
...
11. После изменения стилей проверять, что исходный функционал модулей не удалён или заменён.
