...
- `app.js` остаётся общим приложенческим роутером; существующая функция `loyalty()` из старого кода больше не является рабочим владельцем LOYALTY, потому что `loyalty-ui.js` регистрирует свой маршрут после загрузки общего роутера.
- `loyalty-ui.js` не зависит от декоративного HTML из `app.js` для своих внутренних экранов.
