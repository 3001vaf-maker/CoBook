(function(){
  ...
  function renderView(){if(state.maineView==='clients')return clients();if(state.maineView==='client'){const client=getClient(state.activeClientId);if(client)return clientCard(client);state.maineView='clients';return clients()}return main()}
  function newClient(){const now=new Date().toISOString();const client={id:`client-${Date.now()}-${Math.random().toString(36).slice(2,8)}`,code:nextCode(),name:'',surname:'',gender:'',birthDate:'',phone:'',telegram:'',email:'',programs:{bonus:'',referral:'',other:''},createdAt:now,status:'active',tags:[],notes:'',visits:[],visitCount:0,totalSpent:0};state.clients.push(client);state.activeClientId=client.id;state.maineView='client';saveClients();window.render()}
  function saveClient(id){const client=getClient(id);if(!client)return;const value=id=>document.querySelector(`[data-field="${id}"]`)?.value??'';client.name=value('client-name').trim();client.surname=value('client-surname').trim();client.gender=value('client-gender');client.birthDate=value('client-birth-date');client.phone=value('client-phone').trim();client.telegram=value('client-telegram').trim();client.email=value('client-email').trim();client.programs={bonus:value('client-bonus').trim(),referral:value('client-referral').trim(),other:value('client-other-programs').trim()};client.status=value('client-status')||'active';client.tags=value('client-tags').split(',').map(tag=>tag.trim()).filter(Boolean);client.notes=value('client-notes').trim();saveClients();window.render()}
  function handle(action,e){switch(action){case'clients-open':state.maineView='clients';window.render();break;case'client-new':newClient();break;case'client-open':state.activeClientId=String(e.dataset.clientId||'');state.maineView='client';window.render();break;case'client-save':saveClient(e.dataset.clientId);break;case'client-back':state.maineView='clients';window.render();break;case'maine-back':state.maineView='main';window.render();break}}
