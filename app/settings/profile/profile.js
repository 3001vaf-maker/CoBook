(function(){
  ...
  function render(){const content=section==='home'?home():section==='profession'?profession():section==='work'?work():personal();return shell(`<div class="profile-root">${content}</div>`)}
  function home(){return `<section class="page-head"><div class="eyebrow">НАСТРОЙКИ</div><h1>Профиль</h1><p>Профиль мастера</p></section>
    ...
    <button type="button" class="profile-clean-folder" data-action="profile-work"><b>Работа</b><span>Места работы и рабочая информация</span></button>
  ...
  function profession(){const p=read();return `<section class="page-head"><div class="eyebrow">ПРОФИЛЬ</div><h1>Профессия</h1></section>
    ...
    <button type="button" class="section-back-button" data-action="profile-back">Назад</button>`}
  function work(){const p=read(),ws=Array.isArray(p.workplaces)?p.workplaces:[];return `<section class="page-head"><div class="eyebrow">ПРОФИЛЬ</div><h1>Работа</h1><p>Места работы мастера</p></section>
    ${ws.map((w,i)=>workCard(w,i)).join('')}
    <button type="button" class="profile-clean-folder" data-action="work-add"><b>Добавить место работы</b><span>Создать отдельную карточку рабочего места</span></button>
    ...
    <button type="button" class="section-back-button" data-action="profile-back">Назад</button>`}
  function workCard(w,i){return `<div class="profile-clean-card profile-work-card"><form data-work-form data-index="${i}">
      <div class="profile-work-card-title"><b>Место работы ${i+1}</b><button type="button" class="profile-work-remove" data-action="work-remove" data-index="${i}">Удалить</button></div>
      ...
      <label><span>Валюта</span><button type="button" class="profile-select" data-action="work-currency-select" data-index="${i}"><span>${esc(w.currency||'Выберите валюту')}</span><b>⌄</b></button></label>
  ...
  function handle(action,e){
    ...
    if(action==='profile-work'){section='work';return refresh()}
    ...
    if(action==='work-currency-select')return document.body.insertAdjacentHTML('beforeend',modal('Валюта',currencies,'work-currency-choice',e.dataset.index))
    if(action==='work-currency-choice'){const p=read(),i=+e.dataset.index;p.workplaces=Array.isArray(p.workplaces)?p.workplaces:[];if(p.workplaces[i])p.workplaces[i].currency=e.dataset.value;save(p);e.closest('[data-modal]')?.remove();return refresh()}
    if(action==='work-add'){const p=read();p.workplaces=Array.isArray(p.workplaces)?p.workplaces:[];p.workplaces.push({name:'',city:'',address:'',phone:'',currency:'',schedule:'',links:'',about:''});save(p);return refresh()}
    if(action==='work-remove'){const p=read();p.workplaces=Array.isArray(p.workplaces)?p.workplaces:[];p.workplaces.splice(+e.dataset.index,1);save(p);return refresh()}
    if(action==='profile-back'){if(section!=='home'){section='home';return refresh()}reset();return navigate('settings')}
  }
  document.addEventListener('submit',e=>{const form=e.target.closest('[data-profession-form]');if(form){e.preventDefault();const p=read();p.aboutProfession=String(new FormData(form).get('aboutProfession')||'');save(p);refresh();return}const wf=e.target.closest('[data-work-form]');if(wf){e.preventDefault();const i=+wf.dataset.index,p=read();p.workplaces=Array.isArray(p.workplaces)?p.workplaces:[];if(p.workplaces[i]){const f=new FormData(wf);p.workplaces[i]={...p.workplaces[i],name:String(f.get('name')||''),city:String(f.get('city')||''),address:String(f.get('address')||''),phone:String(f.get('phone')||''),schedule:String(f.get('schedule')||''),links:String(f.get('links')||''),about:String(f.get('about')||'')};save(p)}refresh()}})
