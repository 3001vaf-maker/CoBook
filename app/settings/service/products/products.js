(function(){
  ...
  CoBook.serviceViews.products={
    render(state){
      ...
      return `<section class="ui-list product-list">${state.products.map((p,i)=>{const image=p.image?`<img src="${p.image}" alt="">`:`<span class="product-image-placeholder">▱</span>`;return `<article class="ui-list-row product-card"><button class="ui-list-row-main product-card-main" data-action="edit-product" data-index="${i}" type="button"><span class="product-image">${image}</span><span class="product-card-info"><b>${esc(p.name)}</b><small>${p.price?esc(p.price):'Цена не указана'}</small></span></button><button class="ui-list-delete product-delete" data-action="delete-product" data-index="${i}" type="button" aria-label="Удалить товар">🗑</button></article>`}).join('')}</section><button class="primary full service-add" data-action="open-product" type="button">Добавить товар</button>`;
    }
  };
})();
