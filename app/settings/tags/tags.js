...
(function(){
 ...
 function home(){
   const tags=read();
   return shell(`<section class="page-head"><div class="eyebrow">НАСТРОЙКИ · ЯРЛЫКИ</div><h1>Ярлыки</h1></section><section class="service-list tags-list">${tags.length?tags.map((t,i)=>CoBook.ui.listItem({icon:'●',title:t.name,subtitle:'Пользовательский ярлык',action:'tag-delete',actionLabel:'Удалить',actionAttrs:`data-tag-index="${i}"`,itemClass:'tag-list-item',contentClass:'tag-list-content'})).join(''):'<div class="loyalty-empty">Ярлыков пока нет</div>'}</section><button class="primary full" data-action="tag-create" type="button">Добавить ярлык</button>${back()}`);
 }
 ...
 function handle(action,e){
   if(action==='tag-create')return state.tagsView='editor',refresh();
   if(action==='tag-save'){
