...
(function(){
 ...
 function home(){
   const tags=read();
   return shell(`<section class="page-head"><div class="eyebrow">НАСТРОЙКИ · ЯРЛЫКИ</div><h1>Ярлыки</h1></section><section class="ui-list tags-list">${tags.length?tags.map((t,i)=>`<div class="ui-list-row tags-row"><span class="tag-chip" style="--tag-color:${esc(t.color)}">${esc(t.name)}</span><button class="danger ui-list-delete tags-delete" data-action="tag-delete" data-tag-index="${i}" type="button">Удалить</button></div>`).join(''):'<div class="loyalty-empty">Ярлыков пока нет</div>'}</section><button class="primary full" data-action="tag-create" type="button">Добавить ярлык</button>${back()}`);
 }
