/* CoBook — отдельные экраны клиента: услуга → дата → время → подтверждение → главная */
...
document.addEventListener('click',e=>{
  ...
  if(service){e.preventDefault();e.stopImmediatePropagation();st.service=service.dataset.cflowService;st.date=today();st.month=st.date.slice(0,7);st.slot=null;st.page='selectDate';render();return;}
  const date=e.target.closest('[data-cflow-date]');
  if(date&&!date.disabled){e.preventDefault();e.stopImmediatePropagation();st.date=date.dataset.cflowDate;st.month=st.date.slice(0,7);st.slot=null;st.page='selectTime';render();return;}
  const slot=e.target.closest('[data-cflow-slot]');
  if(slot){e.preventDefault();e.stopImmediatePropagation();st.slot=slot.dataset.cflowSlot;st.page='confirm';render();return;}
  const back=e.target.closest('[data-cflow-back]');
  if(back){e.preventDefault();e.stopImmediatePropagation();st.slot=null;st.page=back.dataset.cflowBack==='service'?'home':'selectDate';render();return;}
  const month=e.target.closest('[data-cflow-month]');
  if(month){e.preventDefault();e.stopImmediatePropagation();const d=D(st.month+'-01');d.setMonth(d.getMonth()+(month.dataset.cflowMonth==='next'?1:-1));st.month=iso(d).slice(0,7);render();return;}
},true);
render();
