(function(){
  ...
  function day(){const date=st.date,h=db.hours[wd(date)]||['10:00','20:00'],start=M(h[0]),end=M(h[1]);let rows='';for(let t=start;t<end;t+=30){const b=db.bookings.find(x=>x.date===date&&x.status!=='cancelled'&&M(x.start)<=t&&M(x.end)>t);if(b&&M(b.start)!==t)continue;rows+=`<div class="day-sheet-row"><div class="day-time">${HM(t)}</div><div class="day-line">${b?`<div class="day-booking ${M(b.end)-M(b.start)>60?'multi':''}" style="height:${Math.max(24,Math.ceil((M(b.end)-M(b.start))/30)*30-6)}px"><b>${esc2(b.name||'Без клиента')} · ${b.start}–${b.end}</b><span>${esc2(b.serviceName||'Услуга')}</span></div>`:`<button class="day-free" data-open-master-booking="${HM(t)}"></button>`}</div></div>`}return shell(`${tabs('day')}<div class="journal-date-switch"><button class="secondary" data-jd="prev">‹</button><div class="date-label">${fmt(date)}</div><button class="secondary" data-jd="next">›</button></div>${isWorking(date)?`<div class="day-sheet">${rows}</div>`:'<button class="day-off-panel" id="open-working-day"><b>Выходной день</b><span>Нажмите, чтобы указать начало и завершение рабочего дня</span></button>'}`,nav())}
  function calendarTab(){return shell(`${tabs('calendar')}<style>.journal-calendar-wrap .calendar-legend,.journal-calendar-wrap .calendar-action-card{display:none!important}.journal-calendar-wrap .weekday:nth-child(6),.journal-calendar-wrap .weekday:nth-child(7){color:#c62828!important}.journal-calendar-wrap .hero{display:none!important}</style><div class="journal-calendar-wrap">${monthCalendar()}</div>`,nav())}
  ...
  window.render=function(){oldRender();if(st.role!=='master')return;if(st.page==='journal')document.getElementById('app').innerHTML=day();if(st.page==='calendar')document.getElementById('app').innerHTML=calendarTab();if(st.page==='journal-list')document.getElementById('app').innerHTML=listTab();document.querySelectorAll('[data-jv]').forEach(b=>b.onclick=()=>{st.page=b.dataset.jv==='day'?'journal':b.dataset.jv==='calendar'?'calendar':'journal-list';window.render()});document.querySelectorAll('[data-jd]').forEach(b=>b.onclick=()=>{st.date=addDays(st.date,b.dataset.jd==='next'?1:-1);window.render()});document.querySelectorAll('[data-open-master-booking]').forEach(b=>b.onclick=()=>booking(b.dataset.openMasterBooking));document.querySelectorAll('[data-toggle-date]').forEach(b=>b.onclick=()=>{db.workingDates[st.date]=!isWorking(st.date);save();window.render()});document.querySelectorAll('[data-date]').forEach(b=>b.onclick=()=>{st.date=b.dataset.date;st.page='journal';window.render()})};
})();
